...
class Injuries extends Component {
    ...
    render() {
        ...
        if (injuriesData.error)
            return (
                <div className="white-bg mt-2">
                    <Errors type="error" message={injuriesData.error} />
                </div>
            );
        return (
            <div>
...
const InjurySummary = props => {
    ...
    return <React.Fragment>{summary.join(', ')}</React.Fragment>;
};
                    ...
                    {data.map((item, index) => {
                        return (
                            <tr key={index}>
                                <td className="">
                                    <span
                                        className={'icon icon-' + item.missingType + ' reason-' + item.missingReasonId}
                                    />
                                </td>
                                <td className="player">
