...
class StandingsOLD extends Component {
    ...
    componentDidMount() {
        ...
        this.getData(
            '/tournament/' + eventData.event.tournament.id + '/' + eventData.event.season.id + '/standings/tables/json'
        );
    }
                        ...
                        <div className="col col-img">
                            <img
                                src={
                                    window.ImageServer +
                                    '/images/u-tournament/' +
                                    standingsTables.tournament.uniqueId +
                                    '.png'
                                }
                                alt={t(standingsTables.tournament.name)}
                            />
                        </div>
                        <div className="col">
                            <div className="name">{t(standingsTables.tournament.name)}</div>
                            <div className="country">
                                <Trans>{standingsTables.category.name}</Trans>
                            </div>
                        </div>
                                ...
                                {standingsTables.tableRows.map((row, index) => {
                                    return (
                                        <tr
                                            key={index}
                                            className={
                                                (row.team.id === eventData.event.homeTeam.id ? 'highlight-home ' : '') +
                                                (row.team.id === eventData.event.awayTeam.id ? 'highlight-away ' : '') +
                                        ...
                                        >
                                            <td
                                                className={
                                                    'order ' +
                                                    (row.promotion && standingsTables.promotionsColoring
                                                        ? 'promotion ' +
                                                          standingsTables.promotionsColoring[row.promotion.id].class
                                                        : '')
                                                }
                                            >
